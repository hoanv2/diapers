...
function metrostore_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'metrostore_custom_header_args', array(
		...
		'flex-height'            => true,
		'flex-width'             => true,
		'wp-head-callback'       => 'metrostore_header_style',
	) ) );
}
add_action( 'after_setup_theme', 'metrostore_custom_header_setup' );

if ( ! function_exists( 'metrostore_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog.
 *
 * @see sparklethemes_custom_header_setup().
 */
function metrostore_header_style() {
